package app.isolvetech.customview
...
//        paint.color = if (fanSpeed == FanSpeed.OFF) Color.GRAY else Color.GREEN
        ...
        pointPosition.computeXYForSpeed(fanSpeed, marker)
        paint.color = Color.BLACK
        canvas?.drawCircle(pointPosition.x, pointPosition.y, radius/14, paint)

        val label = radius + RADIUS_OFFSET_LABEL
        for(i in FanSpeed.values()) {
            pointPosition.computeXYForSpeed(i, label)
